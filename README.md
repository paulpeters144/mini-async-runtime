...
Rust's language-level `async/await` gives you syntax for writing futures and pausing execution, but it does not ship with any mechanism for actually running them. Something has to sit underneath your `async fn`, call `poll` on your futures, park the thread when there's nothing to do, and wake it up when there is. That something is an **async runtime**. In production, it's tokio or async-std, hundreds of thousands of lines of code, built over years by large teams, optimized to within an inch of their lives. They are impressive, but as teaching tools they're not very beginner friendly.

`mar` attempts to be the opposite: a small runtime that genuinely executes async Rust. A few hundred lines, single-threaded in its main execution, with no macros, every piece of boilerplate is written out so that it's visible and learnable rather than hidden behind ergonomic abstractions. No optimization you can't see through. Every component (executor, reactor, timer heap, worker pool) is reduced to its bare minimum.
